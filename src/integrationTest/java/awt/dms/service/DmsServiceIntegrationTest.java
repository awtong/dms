...
class DmsServiceIntegrationTest {
    ...
    class Upload {
        ...
        void when_null_multipart_file_then_throw_constraint_violation_exception() {
            assertThatExceptionOfType(ConstraintViolationException.class).isThrownBy(() -> dmsService.upload(null, Collections.emptyMap()));
        }

        @Test
        void when_null_metadata_then_throw_constraint_violation_exception() {
            assertThatExceptionOfType(ConstraintViolationException.class).isThrownBy(() -> dmsService.upload(MULTIPART_FILE, null));
        }
    }
    ...
    class Find {
        ...
        void when_null_objectid_then_throw_constraint_violation_exception() {
            assertThatExceptionOfType(ConstraintViolationException.class).isThrownBy(() -> dmsService.findOne(null));
        }

        @Test
        void when_not_found_then_return_empty_optional() {
            final Optional<GridFsResource> optional = dmsService.findOne(new ObjectId(Date.from(Instant.now())));
    ...
    class Delete {
        @Test
        void when_null_objectid_then_throw_constraint_violation_exception() {
            assertThatExceptionOfType(ConstraintViolationException.class).isThrownBy(() -> dmsService.delete(null));
        }
    }
}
