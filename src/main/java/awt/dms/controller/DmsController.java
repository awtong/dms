...
public class DmsController {
    ...
    public ResponseEntity<Void> uploadFile(@NotNull @RequestPart("file") final MultipartFile file, @RequestPart final Optional<Map<String, String>> metadata)
            throws IOException {
    ...
    public void getFile(@PathVariable("objectId") final ObjectId objectId, final HttpServletResponse httpServletResponse)
            throws IOException {
        // XXX check user access
    ...
    public ResponseEntity<Collection<FileItem>> getFiles(@RequestParam("user") final String user) {
        ...
        final Collection<FileItem> files = resources.stream().map(
                resource -> new FileItem(Objects.requireNonNull(resource.getFileId()).toString(), resource.getContentType(), resource.getFilename())
